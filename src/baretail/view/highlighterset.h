...
class HighlighterSet {
  public:
    ...
    // baretail: public access to the underlying rule list so our flat-list
    // dialog can edit it without needing klogg's HighlighterSetEdit / friend
    // declarations. Mutating the list invalidates compiledExpression_; call
    // compile() afterwards (or rely on matchLine() to recompile on demand).

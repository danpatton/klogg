...
void AbstractLogView::wheelEvent( QWheelEvent* wheelEvent )
{
    ...
    if ( wheelEvent->modifiers().testFlag( Qt::ControlModifier ) ) {
        const auto angleY = wheelEvent->angleDelta().y();
        if ( angleY != 0 ) {
            Q_EMIT changeFontSize( angleY > 0 );
            return;
        }
    }

    // BareTailPro behaviour: the wheel always scrolls the viewport, even when
    // follow-tail is on. Follow only re-asserts on data updates (see updateData).
    // Skip klogg's snap-to-bottom and pull-to-follow elastic-hook logic entirely.
...
void AbstractLogView::setQuickFindResult( bool hasMatch, const Portion& portion )
{
    if ( portion.isValid() ) {
        ...
        // BareTailPro semantics: when navigating between matches, select the
        // whole line containing the match. The matched substring still gets
        // its yellow QuickFind highlight (drawn by the QFP overlay), so the
        // user sees both: the row is highlighted as the current selection,
        // and the matching word stands out within it.
        selection_.selectLine( portion.line() );
...
{
    // baretail can run without an overview widget; klogg always supplies one,
    // so the original code asserted instead of checking. Guard here so a
    // nullptr OverviewWidget is a supported configuration.
    if ( !overviewWidget_ ) {
